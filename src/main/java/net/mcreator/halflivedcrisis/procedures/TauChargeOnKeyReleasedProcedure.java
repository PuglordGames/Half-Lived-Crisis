		...
		if ((entity.getCapability(HalfLivedCrisisModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new HalfLivedCrisisModVariables.PlayerVariables())).gauss_ammo != 0) {
			if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == HalfLivedCrisisModItems.GAUSS_GUN.get()) {
				if ((entity.getDirection()) == Direction.NORTH) {
					entity.setDeltaMovement(new Vec3(0, 0, (entity.getPersistentData().getDouble("tau_charge") / 30)));
				}
				if ((entity.getDirection()) == Direction.SOUTH) {
					entity.setDeltaMovement(new Vec3(0, 0, (entity.getPersistentData().getDouble("tau_charge") / 30 - (entity.getPersistentData().getDouble("tau_charge") / 30) * 2)));
				}
				if ((entity.getDirection()) == Direction.WEST) {
					entity.setDeltaMovement(new Vec3((entity.getPersistentData().getDouble("tau_charge") / 30), 0, 0));
				}
				if ((entity.getDirection()) == Direction.EAST) {
					entity.setDeltaMovement(new Vec3((entity.getPersistentData().getDouble("tau_charge") / 30 - (entity.getPersistentData().getDouble("tau_charge") / 30) * 2), 0, 0));
				}
