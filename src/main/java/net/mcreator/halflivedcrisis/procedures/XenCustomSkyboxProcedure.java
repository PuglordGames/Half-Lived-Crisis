...
public class XenCustomSkyboxProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != XenCustomSkyboxProcedure.amount || seed != XenCustomSkyboxProcedure.seed) {
			XenCustomSkyboxProcedure.amount = amount;
			XenCustomSkyboxProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!effects.getName().contains("HalfLivedCrisisModDimensionEffects"))
				return;
			if (effects != XenCustomSkyboxProcedure.effects) {
				XenCustomSkyboxProcedure.effects = effects;
				XenCustomSkyboxProcedure.customSky = (List<Predicate<Object[]>>) effects.getField("CUSTOM_SKY").get(null);
			}
			XenCustomSkyboxProcedure.customSky.add(PREDICATE);
		} catch (Exception e) {
		}
	}
